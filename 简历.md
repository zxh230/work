...
项目流程：首先搭建 nfs 服务器用于存放 jenkins和souarquube 的配置文件，在 gitlab 中配置允许 ssh 连接，在 Jenkins 中配置 openjdk 与 maven 的路径并验证，在配置 ssh 连接到 gitlab 服务器，在 gitlab 创建一个空项目并生成 tocken，然后在 jenkins 中使用 tocken 连接 git 项目仓库，将项目上传到 gitlab，在 Jenkins 中创建项目，配置构建后的 maven 打包