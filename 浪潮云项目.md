...
###
apiVersion: v1
kind: PersistentVolume   ##创建一个pv
metadata:
  name: pgsql-nfs  ##pv的名称
  namespace: devops  ##创建到那个命名空间下
spec:
  capacity:
    storage: 5Gi  ##配置pv的大小
  volumeMode: Filesystem  ##挂载类型「文件类型」
  accessModes:
    - ReadWriteMany  ##配置权限
  persistentVolumeReclaimPolicy: Retain  ##回收策略
  storageClassName: "managed-nfs-storage"  ##StorageClassName的名称
  mountOptions:
    - hard
    - nfsvers=4.1
  nfs:  ##挂载方式
    path: /mydata/nfs/pgsql  ##那个目录下
